...
Artificial intelligence is going to redefine what it means to be human. I think in a positive way. 

But we need two things to get there. First, we need to make it dead simple to build intelligent systems<FootnoteRef id="1" />. This means minimizing the number of moving parts needed to build a complete system, then perfecting the few that actually matter.

Second, we need to make the developer experience so focused that builders can spend all their time improving intelligence, and as little time as possible on anything else.

I'm confident this is what AGI will actually look like. It will not come from a single model, at least not any time soon. It will come from perfecting the foundation for building intelligent systems, maximizing the time builders spend using it, and eventually enabling those systems to build and improve themselves.

Astral exists to make this future possible. We're building the engine that powers these systems by rethinking the AI development stack from first principles. But to do that, we have to understand what's currently broken and rethink what's needed to support this new way of building.
...
Eventually I got tired of it. So I started building my own tools just to make my work easier. 
...
This also lets us prove how much leverage the system creates. We've already signed a deal with a late-stage startup to build and maintain a custom AI workflow. It took me, a solo developer, 12 hours to build. The contract pays $15,000 for implementation and $12,000 per month for maintenance. Weekly upkeep takes about 5 hours. That's $1,250 per hour to build, $600 per hour to maintain.
...
<FootnoteSection>
  <Footnote id="1">We define intelligent systems as</Footnote>
